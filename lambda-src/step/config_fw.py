...
def updateRouteNexthop(route, fwMgmtIp, entry_name, api_key, interface, subnetGateway, virtualRouter="default"):
    ...
    xpath = "/config/devices/entry[@name='localhost.localdomain']/network/" \
            "virtual-router/entry[@name='default']/routing-table/ip/static-route/entry[@name='{0}']".format(
                entry_name)
...
def updateRouteNexthopDiscard(route, fwMgmtIp, entry_name, api_key, virtualRouter="default"):
    ...
    xpath = "/config/devices/entry[@name='localhost.localdomain']/network/" \
            "virtual-router/entry[@name='{0}']/routing-table/ip/static-route/entry[@name='{1}']".format(virtualRouter,
                                                                                                        entry_name)
    element = "<destination>{0}</destination><nexthop><discard/></nexthop>".format(
        route)
...
def panEditConfig(fwMgmtIp, api_key, xpath, element):
    ...
    """
    logger.info(
        "Updating edit config with xpath \n{} and element \n{} ".format(xpath, element))
...
def panSetConfig(hostname, api_key, xpath, element):
    ...
    logger.info(
        "Updating set config with xpath \n{} and element \n{} ".format(xpath, element))
...
def getFirewallStatus(gwMgmtIp, api_key):
    ...
    """
    global gcontext
    fwurl = "https://" + gwMgmtIp + \
        "/api/?type=op&cmd=<show><chassis-ready></chassis-ready></show>&key=" + api_key
    ...
    try:
        response = urllib.request.urlopen(
            cmd, data=None, context=gcontext, timeout=5).read()
    ...
    if resp_header.tag != 'response':
        logger.info(
            "[ERROR]: didn't get a valid response from firewall...maybe a timeout")
        return 'cmd_error'
...
def initial_setup_panw_firewall(vpc_summary_route, vpc_cidr_block, fw_mgmt_ip, fw_trust_ip, fw_untrust_ip,
    ...
    updateRouteNexthopDiscard(
        vpc_summary_route, fw_mgmt_ip, entry_name, api_key, virtualRouter="default")
    ...
    editIpObject(fw_mgmt_ip, api_key, 'fw_untrust_sec_int',
                 fw_untrust_sec_ip + '/32')
    ...
    response2 = lib.update_routerId_asn(
        fw_mgmt_ip, api_key, fw_untrust_pub_ip, pa_asn)
...
def lambda_handler(event, context):
    ...
    untrustAZ1_subnet_cidr = lib.find_subnet_by_id(untrustAZ1_subnet)[
        'CidrBlock']
    logger.info('Untrust AZ1 subnet is {}'.format(untrustAZ1_subnet_cidr))
    untrustAZ2_subnet_cidr = lib.find_subnet_by_id(untrustAZ2_subnet)[
        'CidrBlock']
    ...
    panCommit(fw1_mgmt_ip, api_key,
              message="Updated route table and address object")

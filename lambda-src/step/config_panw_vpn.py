...
def updateRouteNexthop(route, hostname, api_key, subnetGateway, virtualRouter="default"):
    ...
    element = "<destination>{0}</destination><interface>ethernet1/2" \
              "</interface><nexthop><ip-address>{1}</ip-address></nexthop>".format(
                  route, subnetGateway)
...
def lambda_handler(event, context):
    ...
    fw1_vpn_status = create_panw_vpn(
        fw1_mgmt_ip, api_key, lambda_bucket_name, fw1_vpnId, 'fw_untrust_int')
    logger.info(
        'called create_panw_vpn {}\n{}\n{}'.format(fw2_trust_ip, lambda_bucket_name, fw2_vpnId, 'fw_untrust_int'))
    fw2_vpn_status = create_panw_vpn(
        fw2_mgmt_ip, api_key, lambda_bucket_name, fw2_vpnId, 'fw_untrust_int')

    logger.info('called create_panw_vpn {}\n{}\n{}'.format(fw1_mgmt_ip, lambda_bucket_name, fw1_sec_vpnId,
                                                           'fw_untrust_sec_int'))
    fw1_sec_vpn_status = create_panw_vpn(
        fw1_mgmt_ip, api_key, lambda_bucket_name, fw1_sec_vpnId, 'fw_untrust_sec_int')
    logger.info('called create_panw_vpn {}\n{}\n{}'.format(fw2_mgmt_ip, lambda_bucket_name, fw2_sec_vpnId,
                                                           'fw_untrust_sec_int'))
    fw2_sec_vpn_status = create_panw_vpn(
        fw2_mgmt_ip, api_key, lambda_bucket_name, fw2_sec_vpnId, 'fw_untrust_sec_int')

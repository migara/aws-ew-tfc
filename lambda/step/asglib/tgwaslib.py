...
def get_device_serial_no(instanceId, gwMgmtIp, fwApiKey):
    ...
    if gwMgmtIp is None:
        logger.error(
            'Firewall IP could not be found. Can not interact with the device')
        return None

    logger.info('Retrieve the serial number from FW {} with IP: {}'.format(
        instanceId, gwMgmtIp))
    ...
    if not serial_no:
        logger.error("Unable to retrieve the serial number from device: {} with IP: {}".format(
            instanceId, gwMgmtIp))
...
def deactivate_fw_license(instanceId, gwMgmtIp, fwApiKey):
    ...
    logger.info('Deactivate and the license for FW: {} with IP: {}'.format(
        instanceId, gwMgmtIp))
...
def handle_license(instanceId, fwMgmtIp, fwApiKey):
    ...
    if not serial_no:
        logger.error(
            'Unable to retrieve the serial no for device with IP: {}'.format(fwMgmtIp))
        return False

    logger.info('The serial number retrieved from device with IP: {} is {}'.format(
        fwMgmtIp, serial_no))
    ...
    try:
        ...
        logger.info(
            "Exception occured while retrieving instance ID information: {}".format(e))
    ...
    if valid_byol:
        for retry in range(1, 10):
            logger.info(
                'Identified the fw license as BYOL. The fw will be de-licensed now.')
            try:
                # TODO
...
def pa_initialize(hostname, api_key, pa_dmz_priv_ip, pa_dmz_pub_ip, pa_asn, pa_dmz_subnet_gw, SubnetCidr,
    ...
    response1 = editIpObject(hostname, api_key, "eth1",
                             "/".join([pa_dmz_priv_ip, mask]))
    ...
    response3 = update_default_route_nexthop(
        hostname, api_key, pa_dmz_subnet_gw)
    logger.info('Response to updateDefaultRouteNextHop {}'.format(response3))
    # Add ApiKey to deactivate License
    response4 = config_deactivate_license_apikey(
        hostname, api_key, license_api_key)
...
def update_routerId_asn(hostname, api_key, routerId, routerAsn, virtualRouter="default"):
    ...
    xpath = "/config/devices/entry[@name='localhost.localdomain']/network/virtual-router/entry[@name='{0}']/protocol/bgp".format(
        virtualRouter)
    element = "<router-id>{0}</router-id><local-as>{1}</local-as>".format(
        routerId, routerAsn)
...
def updateRouteNexthop(route, hostname, api_key, subnetGateway, interface, virtualRouter="default"):
    ...
    element = "<destination>{0}</destination><interface>{1}" \
              "</interface><nexthop><ip-address>{2}</ip-address></nexthop>".format(
                  route, interface, subnetGateway)

    return pan_set_config(hostname, api_key, xpath, element)


def update_bgp_tunnel_ip_pool(ipSegment, tableConn, instanceId, paGroupName, Dryrun=False):
    logger.info('Called updateBgpTunnelIpPool with {} {} {} {}'.format(
        ipSegment, tableConn, instanceId, paGroupName))
    """
    ...
    try:
        ...
        if not Dryrun:
            tableConn.update_item(Key={'IpSegment': ipSegment},
...
def get_available_bgp_tunnel_ip_pool(tableName, instanceId, paGroupName):
    ...
    try:
        logger.info(
            "Fetching BgpTunnelIpPool data with filter status=available")
        table = dynamodb.Table(tableName)
        response = table.scan(FilterExpression=Attr(
            'Available').eq('YES'))['Items']
        if response:
            # Update BgpTunnelIpPool table Attribute "Available"="NO"
            update_bgp_tunnel_ip_pool(
                response[0]['IpSegment'], table, instanceId, paGroupName)
            return response[0]
        else:
            return False
    except Exception as e:
        logger.error(
            "getAvailableBgpTunnelIpPool failed, Error: {}".format(str(e)))
...
def updateVpcTable(tableName, data, status):
    ...
    try:
        ...
        logger.error(
            "Updating Transit VpcTalbe is Failed, Error: {}".format(str(e)))
...
def create_vpn_connection_upload_to_s3(Region, tgwId, cgwId, tunnelOneCidr, tunnelTwoCidr, tag, bucketName,
    ...
    try:
        ...
        ec2Connection.create_tags(
            Resources=[response['VpnConnection']['VpnConnectionId']], Tags=tags)
...
def uploadObjectToS3(vpnConfiguration, bucketName, assumeRoleArn=None):
    ...
    try:
        ...
        if assumeRoleArn:
            ...
            assumedrole = stsConnection.assume_role(
                RoleArn=assumeRoleArn, RoleSessionName="Sample")
        ...
        logger.error(
            "Error uploading file to S3 Bucket, Error : {}".format(str(e)))
        return False


def create_cgw(cgwIp, cgwAsn, Region, tag):
    logger.info('Called createCgw(cgwIp, cgwAsn, Region, tag) {} {} {} {}'.format(
        cgwIp, cgwAsn, Region, tag))
    """Creates CGW and returns CgwId
    """
    try:
        tags = [{'Key': 'Name', 'Value': tag}]
        ec2Connection = boto3.client('ec2', region_name=Region)
        response = ec2Connection.create_customer_gateway(
            BgpAsn=int(cgwAsn), PublicIp=cgwIp, Type='ipsec.1')
        ec2Connection.create_tags(
            Resources=[response['CustomerGateway']['CustomerGatewayId']], Tags=tags)
...
def check_cgw(awsRegion, fwUntrustPubIP, n2Eip):
    logger.info('Called checkCgw(awsRegion, fwUntrustPubIP, n2Eip) {} {} {}'.format(
        awsRegion, fwUntrustPubIP, n2Eip))
    ...
    try:
        ...
        response = ec2_conn.describe_customer_gateways(Filters=filters)[
            'CustomerGateways']
        if response:
            for cgw in response:
...
def pa_configure_vpn(hostname, api_key, vpnConfDict, peerGroup, src_ip, ikeProfile="default", ipsecProfile="default",
    ...
    try:
        ...
        tunnelInfIds = get_free_tunnel_inf_ids(
        ...
        response4 = add_to_peer_group(hostname, api_key, "default", peerGroup,
                                      ...
                                      "".join(
                                          [vpnConfDict['t2_int_ip'], "/30"]),
                                      tunnelInfIds[1], vpnConfDict['t2_int_peer_ip'],
                                      vpnConfDict['vgw_asn'])
        # return response
    except Exception as err:
        logger.info("PA VPN configuration failed: {}".format(str(err)))
        return False
...
def getFirewallStatus(fwIP, api_key):
    ...
    url = "https://%s/api/?type=op&cmd=<show><chassis-ready></chassis-ready></show>&key=%s" % (
        fwip, api_key)
    ...
    try:
        ...
        logger.debug(
            "Got response to 'show chassis status' {}".format(response))
        ...
        if resp_header.tag != 'response':
            logger.debug(
                "Did not get a valid 'response' string...maybe a timeout")
            time.sleep(10)
            return 'cmd_error'
        ...
        if resp_header.attrib['status'] == 'success':
            ...
            for element in resp_header:
                if element.text.rstrip() == 'yes':
                    logger.info(
                        "FW Chassis is ready to accept configuration and connections")
                    return 'yes'
                else:
                    logger.info(
                        "FW Chassis not ready, still waiting for dataplane")
                    time.sleep(10)
                    return 'almost'
...
def getApiKey(hostname, username, password):
    ...
    call = "https://%s/api/?type=keygen&user=%s&password=%s" % (
    ...
    while True:
        try:
            ...
            logger.info(
                "FW Management plane is Responding so checking if Dataplane is ready")
...
def config_deactivate_license_apikey(hostname, api_key, license_api_key):
    ...
    """
    cmd = "<request><license><api-key><set><key>" + \
        license_api_key + "</key></set></api-key></license></request>"
...
def pan_set_config(hostname, api_key, xpath, element):
    ...
    """
    logger.info('Calling pan_set_config {} {} {} {}'.format(
        hostname, api_key, xpath, element))
    data = {
        'type': 'config',
        'action': 'set',
        'key': api_key,
...
def pan_rollback(hostname, api_key, username="admin"):
    ...
    cmd = "<revert><config><partial><admin><member>" + \
        username + "</member></admin></partial></config></revert>"
...
def loadVpnConfigFromS3(bucketName, vpnId):
    ...
    try:
        vpnConf = s3.Object(bucketName, filename).get()[
...
def retrieve_fw_ip(instance_id):
    ...
    logger.info('Eni ID (eth1) for instance : ' +
                instance_id + ' is: ' + eniId)
    try:
        response = ec2_client.describe_network_interfaces(
            NetworkInterfaceIds=[eniId])
    except Exception as e:
        logger.error(
            "[Describe network interfaces failed while retrieving fw ip]: {}".format(e))
        return False
    ...
    if ip.find("NO_") >= 0:
        logger.error('We failed to get either EIP or Private IP for instance: ' +
                     str(instance_id) + ' IP: ' + ip)
        logger.error(
            'We will not proceed further with this Instance: ' + str(instance_id))
        return False
    else:
...
def update_tgw_firewall(vpc_summary_route, fw_trust_ip, fw_untrust_ip, api_key, trustAZ_subnet_cidr, fw_untrust_int):
    ...
    while (True):
        ...
        if err == 'cmd_error':
            ...
            time.sleep(10)
            # continue
            raise FWNotUpException(
                'FW is not up. Nic responds but DP not ready!')
        elif err == 'yes':
...
def find_classic_subnet(kwargs):
    ...
    if len(subnets) > 1:
        raise SystemExit(
            "Error: {} subnets found matching: {}".format(len(subnets), kwargs))
    return subnets[0]
    ...
    while (attachment_count == 0):
        try:
            ...
            if attachment_count > 0:
                ...
                attachment_id = response['TransitGatewayAttachments'][0].get(
...
def create_vpn(fwUntrustPubIP, gwMgmtPubIp, pa_asn, Region, cgw1Tag, table_name, tgwId,
    ...
    response = get_available_bgp_tunnel_ip_pool(
        table_name, instanceId, cgw1Tag)
    N1T2 = response['N1T2']
    N1T1 = response['N1T1']

    vpnId = create_vpn_connection_upload_to_s3(
        Region, tgwId, cgwId, N1T1, N1T2, tag, bucketName, assumeRoleArn=None)
    ...
    logger.info('resource_id is {} from vpnConfDict {}'.format(
        resource_id, vpnConfDict))
...
def terminate_gw(event, tablename):
    ...
    logger.info('Calling release_ips with instance {} from table {}'.format(
        instance_id, tablename))
...
def release_ips(tablename, instance_id):
    ...
    table = dynamodb.Table(tablename)
    logger.info('Releasing instance {} from table {}'.format(
        tablename, strinstance))
...
def update_bgp_table(tableName, vpnId, cgwId, instanceId):
    ...
    try:
        ...
        logger.error(
            "Updating Table {} is Failed, Error: {}".format(tableName, str(e)))
